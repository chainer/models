...
class ElmoLstm(_EncoderBase):
    ...
    def __init__(self,
        ...
        with self.init_scope():
            for layer_index in range(num_layers):
                ...
                setattr(self, 'forward_layer_{}'.format(
                    layer_index), forward_layer)
                setattr(self, 'backward_layer_{}'.format(
                    layer_index), backward_layer)
    ...
    def forward(self,
        ...
        """
        batch_size, total_sequence_length = mask.shape
        stacked_sequence_output, final_states, restoration_indices = \
            self.sort_and_run_forward(self._lstm_forward, inputs, mask)

        num_layers, num_valid, returned_timesteps, encoder_dim = stacked_sequence_output.shape
        ...
        return stacked_sequence_output[:, restoration_indices]
            ...
            for i_layer, lstms in enumerate(
                ...
                for j_direction, lstm in enumerate(lstms):
                    ...
                    for torch_w, tf_w in [[input_weights, tf_input_weights],
                        ...
                        torch_w[(1 * cell_size):(2 * cell_size), :] = \
                            tf_w[(2 * cell_size):(3 * cell_size), :]
                        torch_w[(2 * cell_size):(3 * cell_size), :] = \
                            tf_w[(1 * cell_size):(2 * cell_size), :]

                    lstm.input_linearity.W.data[:] = input_weights
                    lstm.state_linearity.W.data[:] = recurrent_weights
                    lstm.input_linearity.W._requires_grad = requires_grad
                    lstm.state_linearity.W._requires_grad = requires_grad
                    ...
                    lstm.state_linearity.b.data[:] = torch_bias
                    lstm.state_linearity.b._requires_grad = requires_grad

                    # the projection weights
                    proj_weights = numpy.transpose(dataset['W_P_0'][...])
                    lstm.state_projection.W.data[:] = proj_weights
                    lstm.state_projection.W._requires_grad = requires_grad
